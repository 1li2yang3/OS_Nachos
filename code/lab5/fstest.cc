...
void
Append(char *from, char *to, int half)//第一
{
    FILE *fp;
    OpenFile* openFile;
    int amountRead, fileLength;
...
// Append the data in TransferSize chunks
    buffer = new char[TransferSize];
    while ((amountRead = fread(buffer, sizeof(char), TransferSize, fp)) > 0) // 每次以10个字节为单位写入数据   TransferSize 	10 
    {
        int result;
